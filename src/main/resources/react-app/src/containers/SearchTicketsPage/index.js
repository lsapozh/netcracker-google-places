...
class SearchTicketsPage extends Component {
    ...
    getAirportId = name => {
        return airports.find(airport => airport.name === name).id
    }

    loadData = (departureAirport, arrivalAirport) => {
        // const url = `https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/browsedates/v1.0/US/USD/en-US/${departureAirport}/${arrivalAirport}/${
    ...
    getTickets = data => {
        ...
        data.Quotes.map(quote => {
          ...
          const ticket = {price, direct, outboundAirline, inboundAirline, departureDate, returnDate}
         tickets.push(ticket)
        })
      console.log(tickets)
    }
    ...
    onSearch = () => {
        const departureAirport = this.getAirportId(this.state.departureCity)
        const arrivalAirport = this.getAirportId(this.state.arrivalCity)
        this.loadData(departureAirport, arrivalAirport)
    }
